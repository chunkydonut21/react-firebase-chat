...
class UserList extends Component {
  render() {
    const { currentChannelInfo } = this.props
    console.log(currentChannelInfo, 'EHBSDOI')
    return (
      <Col md={5} style={{ background: '#68656947', height: '100vh' }}>
        <div className="user-header">Users {currentChannelInfo?.users.length}</div>
                ...
                renderItem={(item) => (
                  <List.Item>
                    <List.Item.Meta
                      ...
                      description={item.description || 'Hi I am using Snack'}
                    />
                    {currentChannelInfo?.admin !== item.userId ? (
                      <a href="/#" onClick={() => this.props.removeUserFromChannel(currentChannelInfo, item)}>
